...
//de las reglas de la produccion CONTIENE ÚNICAMENTE TERMINALES.
  ...
  for(var i=0;i<this.ls_prod.length;i++){ //El límite es el tamaño de NT que generan producciones
	...
	////////////////////////////////////////////////////////////
  act_ls_nt(this.ls_nt1,this.ls_prod);
////////////////////////////////
...
function act_ls_nt(nt1,ls_prod){
  for(var i=0;i<ls_prod.length;i++){ //El límite es el tamaño de NT que generan producciones
      for(var j=0;j<ls_prod[i].length;j++){
          notlowercase=ls_prod[i][j].toString();
          if(notlowercase != notlowercase.toUpperCase()){
            for(var k = 0; k < notlowercase.length; k++){
               c = notlowercase.charAt(k);
               if(c == c.toUpperCase()){
                  if(nt1.includes(c) && !nt1.includes(ls_nt[i])){
                    this.ls_nt1.splice(i,0,ls_nt[i]);
                    console.log("Lista NT1 ntT");
                    console.log(nt1[i]);
                    act_ls_nt(this.ls_nt1,ls_prod);
                  }
               }
            }
          }
